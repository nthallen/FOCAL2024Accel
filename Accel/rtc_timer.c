 ...
 */
int32_t gds_timer_start(struct timer_descriptor *const descr) {
#ifdef USING_RTC
  ...
  register_value = hri_rtcmode0_read_CTRLA_reg(dev->hw);
  register_value &= ~RTC_MODE0_CTRLA_MATCHCLR;
  hri_rtcmode0_write_CTRLA_reg(dev->hw, register_value);
