...
/* 8/23/2024:
 * This module is copied from uDACS16, where it was originally
 * developed to monitor engine vibrations. It was configured to use a
 * START I2C channel called PMON_I2C, which used SERCOM5, and
 * both are referenced by those names here. I will likely rename
 * them.
 * 8/24/2024:
 * PMON_I2C is now ICM_I2C and SERCOM5 is now SERCOM2
 */

/* In the current configuration, we are using the ICM_I2C to talk to the ICM20948 IMU.
 * In the most recent Atmel START config, this uses SERCOM2, and the clock rate is 10 KHz.
 * This can be changed by defining the symbol CONF_SERCOM_2_I2CM_BAUD, which is currently
 * defined to be 10000. Note, this has to be done in the project config using a -D
 * option in order to read the hal/hpl files. It can be verified in serial_num.h.
 */
...
static void i2c_icm_write(int16_t i2c_addr, const uint8_t *obuf, int16_t nbytes) {
  ...
  i2c_m_async_set_slaveaddr(&ICM_I2C, i2c_addr, I2C_M_SEVEN);
...
static void I2C_ICM_async_error(struct i2c_m_async_desc *const i2c, int32_t error) {
  ...
  if (error == I2C_ERR_BUS) {
    hri_sercomi2cm_write_STATUS_reg(ICM_I2C.device.hw, SERCOM_I2CM_STATUS_BUSERR);
    hri_sercomi2cm_clear_INTFLAG_reg(ICM_I2C.device.hw, I2C_ICM_INTFLAG_ERROR);
  }
}
...
static void i2c_icm_reset() {
  if (!sb_i2c_icm.initialized) {
    ICM_I2C_init();
    i2c_m_async_get_io_descriptor(&ICM_I2C, &I2C_ICM_io);
    i2c_m_async_enable(&ICM_I2C);
    i2c_m_async_register_callback(&ICM_I2C, I2C_M_ASYNC_ERROR, (FUNC_PTR)I2C_ICM_async_error);
    i2c_m_async_register_callback(&ICM_I2C, I2C_M_ASYNC_TX_COMPLETE, (FUNC_PTR)I2C_ICM_txfr_completed);
    i2c_m_async_register_callback(&ICM_I2C, I2C_M_ASYNC_RX_COMPLETE, (FUNC_PTR)I2C_ICM_txfr_completed);
    set_accel_fs(0);
